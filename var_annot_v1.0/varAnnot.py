...
def main():
    ...
    clinData = vcf.Reader(filename=os.path.join(wd,'bin/clinvar.vcf.gz'))
    ...
    with open(outfile,'w') as rf:
        ...
        rf.write("#CHROM\tPOS\tREF\tALT\trsID\tVarClass\tConseqType\tNR\tNV\tABratio\tExacAF\tGene\tENSAnnot"+'\n')
        for var in vcfData:
            if var.CHROM in chr:
                ...
                rf.write('\t'.join([outDict['ID'],outDict['rsid'],outDict['VC'],outDict['type'],outDict['alDP'],outDict['totDP'],outDict['alRatio'],outDict['exac_freq'],'\|'.join(outDict['gene']),'\|'.join([','.join(x) for x in outDict['enstrans'] if x not in ['N/A']]),'\n']))
